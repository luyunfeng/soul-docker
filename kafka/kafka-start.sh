...
KAFKA_HOME=/opt/kafka/kafka_2.12-$KAFKA_VERSION
...
[ ! -z $CONNECT_REST_PORT ] && echo -e "\nrest.port=$CONNECT_REST_PORT" >> /opt/kafka/kafka_2.12-$KAFKA_VERSION/config/connect-standalone.properties && echo -e '\nrest.port='$CONNECT_REST_PORT >> config/connect-distributed.properties
popd
...
then
	...
	ssl_password=soulcoder ; [ ! -z $SSL_PASSWORD ] && ssl_password=$SSL_PASSWORD
...
\nssl.truststore.password="$ssl_password >>  /opt/kafka/kafka_2.12-$KAFKA_VERSION/config/server.properties
	popd
